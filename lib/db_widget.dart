...
class DBWidget extends StatefulWidget {
  ...
  const DBWidget({Key? key, this.dbFuture}) : super(key: key);
...
class _DBWidgetState extends State<DBWidget> {
      ...
      body: FutureBuilder<List<Map<String, dynamic>>?>(
        ...
        builder: (context, snapshot) {
          if (snapshot.hasData &&
            ...
            return Column(
              children: [
                const Text('TABELAS', textAlign: TextAlign.center),
                Expanded(
                  child: Scrollbar(
                            ...
                            Navigator.push(
                              ...
                              MaterialPageRoute(
                                builder: (context) {
                                  ...
                                  tableRows = _db!
                                      .rawQuery(
                                        "SELECT * FROM ${snapshot.data![index]['name']};",
                                      )
                                      .whenComplete(() => setState(() {}));
                                  return _tabela(
                                    tableRows,
  ...
  Widget _tabela(
    Future<List<Map<String, dynamic>>> tableRows,
    String tableName,
  ) {
    return Scaffold(
      appBar: AppBar(title: Text(tableName), elevation: 0),
      body: FutureBuilder<List<Map<String, dynamic>>>(
        future: tableRows,
        builder: (context, snapshot) {
          if (snapshot.hasData &&
            ...
            return PlutoGrid(
              ...
              rows: snapshot.data!
                  .map<PlutoRow>(
                    (row) => PlutoRow(
                      cells: row.map<String, PlutoCell>(
                        (key, value) => MapEntry(key, PlutoCell(value: value)),
                      ),
                    ),
                  )
                  .toList(),
